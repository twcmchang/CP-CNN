...
class VGG16:
    ...
    def build(self, vgg16_npy_path, prof_type=None, conv_pre_training=True, fc_pre_training=True):
        ...
        if os.path.exist(vgg16_npy_path):
    ...
    def sparsity_train(self, l1_gamma=0.001, l1_gamma_diff=0.001, decay=0.0002, keep_prob=0.0):
    ...
    def spareness(self, thresh=0.105
    ...
    def get_conv_filter(self, name, pre_training):
        ...
        if pre_training and name+"_gamma" in self.data_dict.keys(): 
            gamma = tf.get_variable(initializer=self.data_dict[name+"_gamma"], name=name+"_gamma", trainable=False)
        else:
